...
def get_reinitiation_nodes(graph, base_limit=0, upstream_translon_length_limit=100) -> (RDG, list):
    '''
    ...
    for node in graph.get_stop_nodes():
        ...
        if (stop_node.node_start - graph.nodes[stop_node.input_nodes[0]].node_start) > upstream_translon_length_limit:
            continue
...
def plot(
    ...
    :param reinit_base_limit: The length limit for reinitiation
    ...
    if allow_reinitiation:
        reinitiation_nodes = get_reinitiation_nodes(
            graph,
            upstream_translon_length_limit=reinit_base_limit
            )
